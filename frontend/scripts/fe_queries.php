...
class queryFRONTEND{
    ...
    function fetch_employee_name($e_id){
		$query = $this->link->prepare("SELECT CONCAT(PersonalInfo_HR.FirstNames, ' ', PersonalInfo_HR.LastNames) AS NAME FROM `pb_db`.`PersonalInfo_HR` WHERE PersonalInfo_HR.EmployeeId = ?");
     	try{
			$values = array($e_id);
